...
function getStatusText(statusCode) {
    switch(statusCode) {
        ...
        case RQItemStatus.ERR_STOPPED: return "错误已停止";
...
function selectScriptFiles() {
    try {
        var scriptFiles = File.openDialog("请选择一个或多个脚本文件", "*.jsx;*.jsxbin", true);
        if (scriptFiles && scriptFiles.length > 0) {
            var results = [];
            for (var i = 0; i < scriptFiles.length; i++) {
                results.push({
                    path: scriptFiles[i].fsName
                });
            }
            return JSON.stringify({
                scripts: results
...
// [新增] 补充从指定文件夹加载脚本的功能
function scanScriptsInFolder(folderPath) {
    var scripts = [];
    try {
        var folder = new Folder(folderPath);
        if (folder && folder.exists) {
            var files = folder.getFiles("*.jsx");
            var jsxbinFiles = folder.getFiles("*.jsxbin");
            files = files.concat(jsxbinFiles);

            for (var i = 0; i < files.length; i++) {
                if (files[i] instanceof File) {
                     scripts.push({
                        name: decodeURI(files[i].name).replace(/\.(jsx|jsxbin)$/i, ''),
                        path: files[i].fsName
                    });
                }
            }
